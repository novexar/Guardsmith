...
GuardSmith はハイブリッド CI を採用している。メンテナの CI はローカル Docker(`make ci`)で実行し、
GitHub Actions の CI は fork からの外部 PR のみ実行される(同一リポジトリのブランチ PR ではスキップ)。
これは日常 CI で Actions 分数を消費しない方針と、public OSS として外部 PR の自動検証を残す要請を
両立させるための構成である。
